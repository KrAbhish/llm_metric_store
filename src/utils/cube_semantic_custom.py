...
class CubeSemanticLoader(BaseLoader):
    ...
    def _get_dimension_values(self, dimension_name: str) -> List[str]:
        ...
        """
        print("Loading dimension values for: {dimension_name}...")
    ...
    def fetch_raw_cube_metadata(self):
        """Makes a call to Cube's REST API metadata endpoint.

        Returns:
           json formatted documents of metadata
        """
        headers = {
            "Content-Type": "application/json",
            "Authorization": self.cube_api_token,
        ...
        try:
            ...
            print(f"Error occured while fetching metadata from cube endpoint: {e}")
            return None
        
    def format_raw_cube_metadata(self, raw_meta_json):
        cube_data_objects = raw_meta_json.get("cubes", [])
        print(f"Found {len(cube_data_objects)} cube data objects in metadata.")
